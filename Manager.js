...
export class Manager {
     ...
     */
    connect() {
        return __awaiter(this, arguments, void 0, function* (mode = "DATABASE") {
            try {
                ...
                yield db.connect(this.config.url, options);
                this.surreal = db;
            }
            catch (error) {
    ...
    query(query) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                return yield this.surreal.query(query);
            }
            catch (error) {
                const message = error instanceof Error
                    ? error.message
    ...
    use(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const targetNs = (_a = params.namespace) !== null && _a !== void 0 ? _a : this.config.namespace;
                const targetDb = (_b = params.database) !== null && _b !== void 0 ? _b : this.config.database;
                if (!targetNs || !targetDb) {
                    throw new Error("namespace and database must be defined");
                }
                // Update SurrealDB scope
                yield this.surreal.use({
                    namespace: targetNs,
                    database: targetDb,
                });
                // Store new values
     ...
     * }
     */
    defineUser(scope, role, username, password, session, token) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                if (!this.surreal)
                    throw new Error("Not connected to SurrealDB");
                ...
                const query = `DEFINE USER ${username} ON ${scope} PASSWORD '${password}' ROLES ${role}${session && !token
     ...
     * }
     */
    defineParam(name, value, opts, permissions) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                if (!this.surreal)
                    throw new Error("Not connected to SurrealDB");
                const query = `DEFINE PARAM ${opts && opts.overwrite ? `OVERWRITE ` : ""}${opts && opts.ifNotExists ? `IF NOT EXISTS ` : ""}$${name} VALUE ${casting(value)}${permissions && permissions == "NONE"
                    ? ` PERMISSIONS NONE`
