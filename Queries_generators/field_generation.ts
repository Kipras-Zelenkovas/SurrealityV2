...
export const generateFieldQuery = (
    ...
    if (options?.permissions) {
        ...
        if (perms.none) {
            ...
            if (perms.selectExpr) permissionParts.push(`FOR SELECT ${perms.selectExpr}`);
            if (perms.createExpr) permissionParts.push(`FOR CREATE ${perms.createExpr}`);
            if (perms.updateExpr) permissionParts.push(`FOR UPDATE ${perms.updateExpr}`);
            if (perms.deleteExpr) permissionParts.push(`FOR DELETE ${perms.deleteExpr}`);
